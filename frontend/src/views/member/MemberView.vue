...
const payMethods = [
  { key: 'wechat' as const, label: '微信支付', icon: '微' },
  { key: 'alipay' as const, label: '支付宝', icon: '支' },
  { key: 'card' as const, label: '银行卡', icon: '卡' },
]
...
<template>
  <main class="stack-page member-page">
    <!-- Member Header -->
    <a-card :bordered="false" class="member-hero-card">
      <div
        style="
          display: flex;
        ...
        <div style="display: flex; align-items: center; gap: 24px">
          <label style="cursor: pointer; position: relative">
            <a-avatar
              ...
              style="background-color: rgba(255, 255, 255, 0.2); font-size: 32px"
            >
              {{
    ...
    <a-row :gutter="[16, 16]" style="margin-bottom: 24px">
      <a-col :xs="12" :sm="8" :md="6">
        <a-card :bordered="false" class="member-stat-card clickable" @click="openRecharge(100)">
          <WalletOutlined style="font-size: 24px; color: var(--primary); margin-bottom: 8px" />
          <div style="color: #8c8c8c; font-size: 0.9rem">
            余额 <PlusOutlined style="font-size: 12px" />
          </div>
      ...
      <a-col :xs="12" :sm="8" :md="6">
        <a-card :bordered="false" class="member-stat-card">
          ...
          <div style="margin-top: 4px">
            <RouterLink to="/orders" style="font-size: 0.8rem; color: var(--primary)"
              >查看 ›</RouterLink
            >
          </div>
        </a-card>
      ...
      <a-col :xs="12" :sm="8" :md="6">
        <a-card :bordered="false" class="member-stat-card">
          ...
          <strong style="font-size: 1.25rem">{{
            messages.chatUnreadCount + messages.unreadCount
          }}</strong>
        </a-card>
      </a-col>
    </a-row>
      ...
      <a-row :gutter="[16, 16]">
        ...
        <a-col :xs="8" :sm="6" :md="4">
          <RouterLink to="/member/addresses" class="quick-link">
            <EnvironmentOutlined
              style="font-size: 28px; margin-bottom: 12px; color: var(--primary)"
            />
            <span>收货地址</span>
          </RouterLink>
        </a-col>
    ...
    <div v-if="showRecharge" class="modal-overlay" @click.self="showRecharge = false">
      <div class="modal">
        ...
        <div class="modal-actions">
          ...
          <a-button type="primary" @click="goPay"> 下一步 </a-button>
        </div>
      </div>
    </div>
