...
interface Dashboard {
  totalUsers: number; totalOrders: number; totalProducts: number; totalSales: number
...
onMounted(async () => {
  try {
    ...
    data.value = {
      totalUsers: 128, totalOrders: 356, totalProducts: 48, totalSales: 125680,
      todayOrders: 12, todaySales: 4680,
      trend: [
...
const trendOption = computed(() => ({
  ...
  series: [
    { name: '订单数', type: 'bar', data: data.value?.trend.map(t => t.orders) || [], itemStyle: { color: '#10b981' } },
    { name: '销售额(元)', type: 'line', yAxisIndex: 1, data: data.value?.trend.map(t => t.sales) || [], itemStyle: { color: '#f59e0b' } },
  ],
}))
...
const pieOption = computed(() => ({
  ...
  series: [{
    ...
    data: Object.entries(data.value?.statusCount || {}).map(([k, v]) => ({
      name: { pending_payment: '待付款', paid: '已付款', shipping: '配送中', completed: '已完成', cancelled: '已取消' }[k] || k,
      value: v,
    })),
    ...
    <a-spin :spinning="loading">
      ...
      <a-row :gutter="16" style="margin-bottom: 24px;">
        <a-col :span="6" v-for="card in [
          { label: '总用户', value: data?.totalUsers, icon: '👥' },
          { label: '总订单', value: data?.totalOrders, icon: '📦' },
          { label: '总商品', value: data?.totalProducts, icon: '🏷️' },
          { label: '总销售额', value: '￥' + (data?.totalSales || 0).toLocaleString(), icon: '💰' },
        ]" :key="card.label">
