      ...
      <div class="checkout-main">
        <section class="panel">
          ...
          <article v-if="member.defaultAddress" class="address-card selected">
            <strong style="display: block; margin-bottom: 4px; font-size: 1rem"
              >{{ member.defaultAddress.name }} {{ member.defaultAddress.phone }}</strong
            >
        ...
        <section class="panel">
          ...
          <a-radio-group
            v-model:value="orders.deliveryMethod"
            option-type="button"
            button-style="solid"
            size="large"
            @change="loadPreview"
        ...
        <section class="panel">
          ...
          <a-form layout="vertical">
            <a-form-item label="优惠券">
              <a-select
                v-model:value="orders.selectedCouponId"
              ...
              >
                ...
                <a-select-option
                  v-for="coupon in member.availableCoupons"
                  :key="coupon.id"
                  :value="coupon.id"
                >
            ...
            <a-form-item>
              <a-checkbox
                v-model:checked="orders.usePoints"
              ...
              <div style="color: #8c8c8c; font-size: 0.8rem; margin-top: 2px">
                ...
                <span v-if="member.points < 100" style="color: var(--danger)"
                  >（积分不足，无法使用）</span
                >
              </div>
            </a-form-item>
      ...
      <aside class="summary-panel">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px; margin-bottom: 24px">
          <div style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">商品金额</span
            ><strong style="font-size: 1.1rem">￥{{ price.subtotal.toFixed(2) }}</strong>
          </div>
          <div
            v-if="price.memberDiscount > 0"
            style="display: flex; justify-content: space-between"
          >
            <span style="color: #8c8c8c">会员折扣 ({{ member.level }})</span
            ><strong style="color: var(--danger); font-size: 1.1rem"
              >-￥{{ price.memberDiscount.toFixed(2) }}</strong
            >
          </div>
          <div style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">优惠券</span
            ><strong style="color: var(--danger); font-size: 1.1rem"
              >-￥{{ price.couponAmount.toFixed(2) }}</strong
            >
          </div>
          <div style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">积分抵扣</span
            ><strong style="color: var(--danger); font-size: 1.1rem"
              >-￥{{ price.pointsAmount.toFixed(2) }}</strong
            >
          </div>
          <div style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">礼品卡</span
            ><strong style="color: var(--danger); font-size: 1.1rem"
              >-￥{{ price.giftCardAmount.toFixed(2) }}</strong
            >
          </div>
          <div v-if="price.balanceAmount > 0" style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">余额支付</span
            ><strong style="color: var(--danger); font-size: 1.1rem"
              >-￥{{ price.balanceAmount.toFixed(2) }}</strong
            >
          </div>
          <div style="display: flex; justify-content: space-between">
            <span style="color: #8c8c8c">运费</span
            ><strong style="font-size: 1.1rem">￥{{ price.freight.toFixed(2) }}</strong>
          </div>
        </div>
        <div style="border-top: 1px solid #f0f0f0; margin-bottom: 24px; padding-top: 16px">
          <div style="display: flex; justify-content: space-between; align-items: baseline">
            ...
            <strong style="color: var(--price); font-size: 1.75rem"
              >￥{{ price.total.toFixed(2) }}</strong
            >
          </div>
        </div>
        <a-button
