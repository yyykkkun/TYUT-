...
function setDelivery(method: 'platform' | 'third-party' | 'pickup') {
    ...
    <section v-else class="checkout-grid">
      <div class="checkout-main">
        ...
        <section class="panel">
          ...
          <div class="segmented">
            <button
              :class="{ active: orders.deliveryMethod === 'platform' }"
              @click="
                orders.deliveryMethod = 'platform'
                loadPreview()
              "
            >
              平台物流
            ...
            <button
              :class="{ active: orders.deliveryMethod === 'third-party' }"
              @click="
                orders.deliveryMethod = 'third-party'
                loadPreview()
              "
            >
              第三方物流
            ...
            <button
              :class="{ active: orders.deliveryMethod === 'pickup' }"
              @click="
      ...
      <aside class="summary-panel">
        <h2>金额明细</h2>
        <p>
          <span>商品金额</span><strong>￥{{ price.subtotal }}</strong>
        </p>
        <p>
          <span>优惠券</span><strong>-￥{{ price.couponAmount }}</strong>
        </p>
        <p>
          <span>积分抵扣</span><strong>-￥{{ price.pointsAmount }}</strong>
        </p>
        <p>
          <span>礼品卡</span><strong>-￥{{ price.giftCardAmount }}</strong>
        </p>
        <p>
          <span>运费</span><strong>￥{{ price.freight }}</strong>
        </p>
        <p class="total">
          <span>应付</span><strong>￥{{ price.total }}</strong>
        </p>
        <button class="btn primary block" type="button" :disabled="submitting" @click="submit">
          提交订单
        </button>
      </aside>
    </section>
